...
extern "C" void app_main() {
	...
	pthread_attr_init(&(task_list[1].pthread_attr));
	pthread_create(&(task_list[1].pthread), &(task_list[1].pthread_attr), task_list[1].task, NULL);
	pthread_join(task_list[1].pthread, NULL);
...
#if (AK_PRIORITY_ENABLE == 1)
		int status = pthread_attr_setschedpolicy(&(task_list[index].pthread_attr), SCHED_FIFO);
		if (status != 0) {
